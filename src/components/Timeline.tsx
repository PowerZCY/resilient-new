...
export default function Timeline(): JSX.Element {
  ...
  useEffect(() => {
    ...
    setIsIndicatorRefReady(false); // Reset indicator ready state
  ...
  const loadProgressPosition = useCallback(() => {
    ...
    // Ensure ref is ready before attempting to load
    if (!progressIndicatorRef.current) {
      console.warn("Attempted to load progress position, but ref is not ready.");
  ...
  }, []); // Removed dependency on ref, check happens inside
      ...
      <AnimatePresence>
        {(initialLoadDone.current || entries.length > 0 || loading) && nickname && (
          <motion.div
            ...
            style={{ position: 'fixed' }} // Keep fixed position
